...
const FilterTable = ({ data }) => {
	...
	const handleActiveUnActiveToggle = async (bannerId) => {
		...
		try {
			const response = await axios.put("http://45.198.14.69:3000/api/admin/activeUnActiveBanner",
				{ bannerId: bannerId},
				{
					headers: {
			...
			if (response.status === 200) {
				...
				const updatedBanners = await Promise.all(filteredBanner.map(async (banner) => {
					if (banner.id === bannerId) { banner.isActive = !banner.isActive; }
					return banner;
				}));
		...
		<motion.div className='bg-gray-800 bg-opacity-50 backdrop-blur-md shadow-lg rounded-xl p-6 border border-gray-700' initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
			...
			<div className='overflow-x-auto'>
				<table className='min-w-full divide-y divide-gray-700'>
					<thead>
						<tr>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Banner ID </th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Banner Name</th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Total Imges </th>
					...
					<tbody className='divide divide-gray-700'>
						{filteredBanner.map((banner) => (
							...
							<motion.tr key={banner.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>

								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center hover:text-green-400'>{banner.id}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{banner.bannerName}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{banner.imageUrl.length}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{banner.isActive ? "true": "false"}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{new Date(banner.createdAt).toLocaleDateString()}</td>

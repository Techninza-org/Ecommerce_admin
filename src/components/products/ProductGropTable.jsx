  ...
  const handleStatusToggle = async (productId) => {
    try {
      ...
      const response = await axios.put(
        `http://45.198.14.69/api/seller/ActiveInactiveProductGroup/${productId}`,
        {},
        {
          headers: {
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://45.198.14.69/api/seller/deleteProductGroup/${productId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.status === 200) {
        const updatedProductGroup = productGroupList.filter(
          (productGroup) => productGroup.id !== productId
        );
        setProductGroupList(updatedProductGroup);
      } else {
          ...
          <tbody className="divide divide-gray-700">
            {productGroupList.map((productGroup) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center">
                  {productGroup.groupDesc}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center">
                  {productGroup.isActive ? "Active" : "In-Active"}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center">
                  <button onClick={() => handleStatusToggle(productGroup.id)}>
                    {" "}
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center">
                  <button
                    className="text-white-400 hover:text-red-600"
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center">
                  <button
                    className="text-white-400 hover:text-blue-300"
                    onClick={() => handleProductGroupView(productGroup)}
                  >
                    {" "}
                    {<Eye />}
                  </button>

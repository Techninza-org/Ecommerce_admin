...
const OrdersTable = ({ data }) => {
	...
	const handleStatusChange = async (orderId, newStatus) => {
		...
		const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhZG1pbklkIjoxLCJpYXQiOjE3MzU4ODI4NzcsImV4cCI6MTczNjc0Njg3N30.TAvxUdkkYT0mnPDKF-EIuySREq3YMoctuAcnOIQwCQk"
		try {
			const response = await axios.put("http://45.198.14.69:3000/api/seller/updateOrderStatus",
				{ orderId, newStatus },
				{
					headers: {
	...
	const handleRedirectToOrderPage = (order) => {
		...
		<motion.div className='bg-gray-800 bg-opacity-50 backdrop-blur-md shadow-lg rounded-xl p-6 border border-gray-700' initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
			<div className='flex justify-between items-center mb-6'>
				...
				<div className='relative'>
					<input type='text' placeholder='Search orders...' className='bg-gray-700 text-white placeholder-gray-400 rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500' value={searchTerm} onChange={handleSearch} />
				...
				<table className='min-w-full divide-y divide-gray-700'>
					<thead>
						<tr>
							...
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Customer ID</th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Address ID</th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> COD </th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Customer Name</th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Order Amount </th>
							...
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Pincode </th>
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider text-center'> Change Status </th>
					...
					<tbody className='divide divide-gray-700'>
						{filteredOrders.map((order) => (
							...
							<motion.tr key={order.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
								...
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{order.user.id}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{order.orderAddress.id}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{String(order.isCod)}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>{order.user.name}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100 text-center'>${order.totalAmount.toFixed(2)}</td>
								...
								<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300 text-center'>{new Date(order.createdAt).toLocaleDateString()}</td>
								<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300 text-center'>{order.orderAddress.pincode}</td>
								<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 text-center">
									<select className="bg-gray-700 text-white rounded-lg px-2 py-1 text-center" onChange={(e) => handleStatusChange(order.id, e.target.value)}>

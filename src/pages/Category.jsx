...
const Category = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = parentId
        ? { childrenCategoryName: categoryName, parentId: parseInt(parentId, 10) }
        : formData;
  ...
  const handleDeleteCategory = async (e) => {
    ...
    try {
      const response = await axios.delete("http://45.198.14.69:3000/api/admin/deleteCategoryById", {
        headers: {
      ...
      // Update category list
      setCategories(categories.filter((category) => category.id !== parseInt(categoryId, 10)));
    } catch (error) {
                ...
                {categories.map((category) => (
                  <>
                    <option key={category.id} value={category.id}>
                      {category.parentId === null ? "• " : ""} {/* Bullet for parent categories */}
                      {category.categoryName}
                    </option>
                    {renderCategoryName(category, " > ")} {/* Render child categories with hierarchy */}
                  </>
                ))}
              </select>
            </div>

...
const ProductGroupPage = () => {
    ...
    useEffect(() => {
        const fetchProductGroup = async () => {
            try {
                const response = await axios.get(`${APIs.BASE_URL_FOR_API}api/seller/getAllProductGroups`, {
                    headers: {
	...
	useEffect(() => {
		const fetchProducts = async () => {
			try {
				const response = await axios.get(`${APIs.BASE_URL_FOR_API}api/admin/getAllProducts`, {
					headers: {
						Authorization: `Bearer ${Cookies.get("token")}`,
					},
				});
    ...
    useEffect(() => { // values updating immediately
	...
	return (
		<div className="flex-1 overflow-auto relative z-10">
			...
			<main className="max-w-7xl mx-auto py-6 px-4 lg:px-8">
				...
				>
					<h3 className="text-xl font-semibold text-gray-100 mb-4">Add New Product Group</h3>
							...
							>
								...
								{products.map((product) => (
									<option key={product.id} value={product.id}>
										{product.name} {/* Replace `name` with the appropriate field */}
									</option>
								))}
							</select>
						</div>

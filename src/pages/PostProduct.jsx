...
const PostProductsPage = () => {
  ...
  const [gstOptions, setGstOptions] = useState(["18", "9", "3", "6", "28", "12", "5"]);
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 lg:px-8">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-200 mb-2"> Description </label>
              <textarea name="productDescription" value={formData.productDescription} onChange={handleInputChange} required placeholder="Enter product description" rows={2} className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            ...
            <div className="flex space-x-4 mb-4">
              ...
              <div>
                ...
                <select
                  ...
                  // className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 scrollbar-hide"
                  style={{
                    width: "100%",
                    ...
                    scrollbarColor: "#3b82f6 #4b5563", // Thumb and track colors for Firefox
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  // className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  // className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-blue-600"
                  style={{
                    width: "100%",
                    backgroundColor: "#374151",
                    color: "#fff",
            ...
            <div className="flex space-x-4 mb-4">
              <div className="w-1/2">
                ...
                >
                  ...
                  {gstOptions.map((gst, index) => (<option key={index} value={gst}> {gst}% </option>))}
                </select>
              </div>
              ...
              <div className="w-1/2">
                ...
                <div className="flex">
                  <input type="text" value={customGst} onChange={(e) => setCustomGst(e.target.value)} placeholder="Enter GST" className="flex-grow bg-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
                  <button type="button" onClick={handleAddGstOption} className="ml-2 bg-blue-500 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-600 transition"> Add </button>
                </div>
            ...
            {/* <div className="flex space-x-4 mb-2">
              <label className="w-full block text-sm text-gray-200">
                <input type="number" placeholder="Price" className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
              </label>
            </div> */}

            {/* selling price */}
            <div>
              <label className="block text-sm font-medium text-gray-200 mb-2"> Selling Price </label>
              <div className="flex items-center space-x-4">

                <label className="flex items-center text-gray-200">
                  <input type="radio" name="sellingPriceToggle" value="yes" onChange={(e) => handleSellingPriceToggle(e.target.value)} checked={showSellingPrice} className="form-radio text-blue-500" />
                  <span className="ml-2">On</span>
                </label>

                <label className="flex items-center text-gray-200">
                  <input type="radio" name="sellingPriceToggle" value="no" onChange={(e) => handleSellingPriceToggle(e.target.value)} checked={!showSellingPrice} className="form-radio text-blue-500" />
            ...
            {formData.attributesJson.map((attribute, index) => (
              <div key={index}>
                ...
                <div className="mb-2 flex">
                  ...
                  <input type="number" placeholder="salePrice" className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 ml-2" />

                  <input type="number" placeholder="quantity" className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 ml-2" />

                </div>

                {attribute.fields.map((field, fieldIndex) => (
                  // <div key={fieldIndex} className="flex space-x-4 mb-2">
                  <div key={fieldIndex} className="space-x-4 mb-2">
                    {/* Field Key */}
                    <select
                      value={field.name} onChange={(e) => handleNestedChange(index, fieldIndex, "name", e.target.value)} className="flex-1 bg-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500">
                      {/* <option value="">Select Field Key</option> */}
                      <option value="">Select Varient</option>
                      {availableFields.map(({ keyName }) => (<option key={keyName} value={keyName}> {keyName} </option>))}
                    </select>

                    {/* Field Value */}
                    <select value={field.value} onChange={(e) => handleNestedChange(index, fieldIndex, "value", e.target.value)} className="flex-1 bg-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500">
                      {/* <option value="">Select Field Value</option> */}
                      <option value="">Select Varient Value</option>
                      {availableFields
                        .find(({ keyName }) => keyName === field.name)
                        ?.fieldValues.map(({ valueName }) => (
                          <option key={valueName} value={valueName}> {valueName} </option>
                        ))}
                    </select>
            ...
            {/* Add Attribute */}
            {/* <button type="button" onClick={handleAddAttribute} className="text-blue-500 text-sm"> + Add Attribute Group </button> */}
            <button type="button" onClick={handleAddAttribute} className="text-blue-500 text-sm"> + Add Varient </button>

            <div>
              <label className="block text-sm font-medium text-gray-200 mb-2"> Upload Images </label>
              <input type="file" multiple onChange={handleFileChange} className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>

            <div>
              <button type="submit" className="w-1/3 bg-blue-500 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-600 transition focus:outline-none focus:ring-2 focus:ring-blue-400" > {" "} Post Product{" "} </button>
            </div>
